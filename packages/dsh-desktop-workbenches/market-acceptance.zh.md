...
- 工作台只能通过宿主提供的工作台接口创建或恢复绑定会话。前台工作台中的原生工作区切换由宿主在目标工作区创建新的 owner 会话并在显示前完成绑定；全局原生新建或打开普通会话后必须允许宿主退出工作台。工作台不得用覆盖层、独立路由或自行恢复状态收编普通会话或把用户拉回工作台。
...
- [ ] 前台工作台中的原生工作区切换会新建并预先绑定当前 owner 会话，不复用已有普通或其他 owner 会话；全局新建和打开普通会话仍退出工作台并保持未绑定。
