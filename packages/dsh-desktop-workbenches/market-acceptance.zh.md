...
- 工作台只能通过宿主提供的工作台接口创建或恢复绑定会话。原生会话与各工作台是平行模式：工作台模式的新建会话和工作区切换只创建或打开当前 owner 会话，原生模式只创建或打开未绑定会话，且不得按工作区中已有会话反推或切换工作台。工作台不得用覆盖层、独立路由或自行恢复状态收编普通会话或把用户拉回工作台。
- 切换工作台时宿主不会自动收起或展开左侧会话栏，工作台不得控制用户的侧栏状态，也不得用自己的覆盖层替代宿主侧栏。绑定会话应由宿主在标题前显示对应工作台图标，工作台必须提供可辨识且适合小尺寸显示的图标。
...
- [ ] 工作台模式的新建会话和工作区切换只创建或打开当前 owner 会话；原生模式只创建或打开未绑定会话，不会因工作区中存在工作台会话而自动切换模式。
- [ ] 切换工作台不会自动改变会话栏展开状态；折叠态和展开态均无裁切，绑定会话显示正确的工作台图标，普通会话不显示。
