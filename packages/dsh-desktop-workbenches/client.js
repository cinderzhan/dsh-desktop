      ...
      return h('nav', { className: 'dshWb dshWbSidebarSwitcher', 'data-dsh-workbench-switcher': '', 'aria-label': '切换会话模式', onBlur: (event) => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false) }, onKeyDown: (event) => { if (event.key === 'Escape') { setOpen(false); event.currentTarget.querySelector('.dshWbModeSelect')?.focus() } } },
        ...
        open && h('div', { className: 'dshWbModeMenu', role: 'menu', 'aria-label': '选择会话模式' },
          ...
          ...pinned.map((item, index) => h('div', { key: item.id, className: 'dshWbModeOptionRow', role: 'none' },
            ...
            h('span', { className: 'dshWbModeOrder', role: 'group', 'aria-label': `调整${item.title}顺序` },
              h('button', { type: 'button', className: 'dshWbModeOrderButton', title: `上移${item.title}`, 'aria-label': `上移${item.title}`, disabled: index === 0, onClick: () => service.run(service.reorder(item.id, pinned[index - 1].id)) }, h(MarketIcon, { name: 'chevronUp', size: 13 })),
              h('button', { type: 'button', className: 'dshWbModeOrderButton', title: `下移${item.title}`, 'aria-label': `下移${item.title}`, disabled: index === pinned.length - 1, onClick: () => service.run(service.reorder(item.id, pinned[index + 2]?.id ?? null)) }, h(MarketIcon, { name: 'chevronDown', size: 13 })))))),
