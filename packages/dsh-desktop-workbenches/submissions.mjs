...
const V1_BACKUP_NAME = 'submissions.v1.backup.json'
const BASE_ENTRY_FIELDS = ['id', 'title', 'description', 'author', 'repository', 'screenshot', 'packageSha256', 'packageBytes', 'status', 'createdAt']
const V2_ENTRY_FIELDS = [...BASE_ENTRY_FIELDS, 'pullRequestUrl', 'lastRemoteStatus', 'lastSyncedAt', 'exportedAt', 'specVersion']
...
function validatePullRequestUrl(value) {
  ...
  const parts = url.pathname.split('/').filter(Boolean)
  return parts.length === 4 && /^[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?$/.test(parts[0])
    && /^[A-Za-z0-9._-]{1,100}$/.test(parts[1]) && parts[2] === 'pull' && /^[1-9][0-9]*$/.test(parts[3])
}

function validateStored(value, version) {
  const fields = version === 1 ? BASE_ENTRY_FIELDS : V2_ENTRY_FIELDS
  const expectedStatus = version === 1 ? 'pending' : 'local-draft'
  if (!isObject(value) || value.version !== version || Object.keys(value).some(key => !['version', 'submissions'].includes(key))
    || !Array.isArray(value.submissions)) throw new Error('Invalid submissions')
  for (const entry of value.submissions) {
    if (!isObject(entry) || Object.keys(entry).some(key => !fields.includes(key))
      || typeof entry.id !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(entry.id)
      || !validateIsoTimestamp(entry.createdAt) || entry.status !== expectedStatus) throw new Error('Invalid submission')
    if (version === 2 && ((hasOwn(entry, 'pullRequestUrl') && !validatePullRequestUrl(entry.pullRequestUrl))
      || (hasOwn(entry, 'lastRemoteStatus') && (typeof entry.lastRemoteStatus !== 'string' || !/^[a-z][a-z0-9-]{0,63}$/.test(entry.lastRemoteStatus)))
      || (hasOwn(entry, 'lastSyncedAt') && !validateIsoTimestamp(entry.lastSyncedAt))
      || (hasOwn(entry, 'exportedAt') && !validateIsoTimestamp(entry.exportedAt))
      || (hasOwn(entry, 'specVersion') && (typeof entry.specVersion !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(entry.specVersion))))) {
      throw new Error('Invalid remote submission metadata')
    }
    const checked = {
    ...
    if (hasOwn(entry, 'packageSha256') !== hasOwn(entry, 'packageBytes')
      || (hasOwn(entry, 'packageSha256') && (!/^[a-f0-9]{64}$/.test(entry.packageSha256) || !Number.isInteger(entry.packageBytes)
        || entry.packageBytes < 1 || entry.packageBytes > MAX_PACKAGE_BYTES))) throw new Error('Invalid package metadata')
...
export function createSubmissionStore(root) {
  ...
  const backupPath = join(root, V1_BACKUP_NAME)
  ...
  async function readFileState() {
    ...
    try {
      ...
      if (error.code === 'ENOENT') return { version: 2, submissions: [] }
    ...
    try {
      ...
      const state = validateStored(parsed, isV1 ? 1 : 2)
      ...
      if (!isV1) return state
      const migrated = { version: 2, submissions: state.submissions.map(entry => ({ ...entry, status: 'local-draft' })) }
      try {
    ...
    create: input => {
      ...
      return queue(async () => {
        ...
        const saved = { id: randomUUID(), ...fields,
          ...
          status: 'local-draft', createdAt: new Date().toISOString() }
        const next = { version: 2, submissions: [...current.submissions, saved] }
        await mkdir(root, { recursive: true })
        ...
        try {
          ...
          throw error
        } finally { if (packageTemporary) await rm(packageTemporary, { force: true }) }
