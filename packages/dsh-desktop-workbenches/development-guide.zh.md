...
先从项目读取名称、作者、版本、说明和测试命令，只询问无法查明且会影响实现的必要信息。开发请求本身不代表授权公开代码、商业数据或凭证。
...
- `workbench.json` 和自定义 `workbenchId` 不是工作台协议的一部分；运行时包名必须与 `window.__ModuleLoader__.load({ id })` 对应。
...
- [ ] 工作台出现在“已安装的工作台”和顶部模式切换器中，能打开，名称正确；“工作台管理”入口常驻右侧，运行的就是刚构建的版本。
- [ ] 原生会话与各工作台是平行模式；工作台模式的新建会话和工作区切换只创建或打开当前 owner 会话，原生模式只创建或打开未绑定会话，且不会按工作区内容自动切换工作台。
- [ ] 切换模式不会自动收起或展开会话栏；折叠态与展开态的按钮、菜单、选中态和焦点轮廓完整可见，没有裁切或重叠。
