...
- [ ] 第 3 节全部“必须”项通过；`pnpm pack` 的产物包含声明的服务端入口、客户端入口和 `cordis.patch.yml`，并能从该产物安装。

**安装与运行**

- [ ] 安装到本机并重启后能加载；`dsh --profile web --dump-config` 能看到本包的插件层。
- [ ] 第 4–7 节逐项实测通过，包括业务面板边界、目录选择、会话归属、模式切换、侧栏状态和工作台图标。
- [ ] 切换、关闭、重开及卸载工作台后，用户会话、项目文件、草稿、笔记和收藏按规范保留。
