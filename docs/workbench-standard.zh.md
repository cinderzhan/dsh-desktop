...
工作台内部的原生会话区应支持明确执行“在此工作区新建工作台会话”或“选择资料位置并开始工作台会话”。这类动作会创建会话并登记当前工作台归属。需要本地目录时，必须先调用宿主提供的目录选择器，让用户选择已有目录，或由用户在系统选择器中明确新建目录；工作台不得在默认位置、用户主目录、当前仓库或其他推测路径中静默创建文件夹。目录选择完成前不得创建工作区、会话或绑定；用户取消时不得留下目录、空项目或会话。失败可重试，实现不得默默改用无关工作区。

顶部模式切换器中的“原生会话”和各工作台是平行模式，当前模式决定会话的可见范围与新建归属。在工作台模式点击“新建会话”或切换工作区时，只能创建或打开属于当前工作台的会话，并在显示为 current 前完成绑定；不得复用、收编或改绑目标工作区中的普通会话及其他工作台会话。在原生会话模式点击“新建会话”或切换工作区时，只能创建或打开未绑定会话；不得因为该工作区存在工作台会话就自动切换工作台。只有用户显式选择某个工作台模式，或打开一条已绑定会话时，才进入对应工作台。
...
宿主在侧栏顶部提供单一模式切换器：按钮只显示当前模式，展开下拉菜单后可在“原生会话”和已添加工作台之间切换；“工作台管理”入口常驻最右侧。工作台不得自行复制、遮挡或替换这些公共入口。切换模式只改变前台展示和会话筛选，不执行卸载重装，也不停止已有后台任务。

切换工作台不得自动收起或展开左侧会话栏；侧栏宽窄由用户控制并在模式间保持。折叠态与展开态都必须保证按钮、选中态、下拉菜单和焦点轮廓不被裁切、重叠或溢出窗口。
...
工作区侧边栏中的切换和“新建会话”都保留 DSH 原生入口和用户选择的目标，但按当前模式执行。在工作台模式中，新建与工作区切换只显示当前 owner 的会话；目标工作区没有这类会话时创建新的 owner 会话，并先持久化归属和最近会话再显示。在原生会话模式中，只显示未绑定会话；目标工作区没有未绑定会话时创建一条，不能从同工作区已有会话推断或切换到任何工作台。异步创建完成前若用户继续导航、切换模式或移除 owner，新会话不得绑定到失效 owner，也不得把界面拉回旧模式或覆盖用户后续选择。

显式打开普通未绑定会话时，宿主切换到“原生会话”模式；显式打开已绑定且 owner 可用的会话时，切换到对应工作台。历史 owner 已移除、卸载或暂时不可用的会话按普通会话处理，不自动唤起或重装 owner。这个导航只改变前台界面状态，不删除工作台最近会话、业务资料、项目文件、收藏或笔记。
