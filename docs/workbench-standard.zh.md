...
用户从工作台市场复制一段投稿 Prompt，交给正在开发该工作台的 Agent。Agent 应先完成开发与校验，再从项目中核实工作台名称、描述、作者名称和精确的 GitHub HTTPS 仓库地址；只有无法从项目确认的信息才询问用户。该地址只用于标识待审核源码；当前版本不会登录 GitHub、向 GitHub 上传或修改文件，也不会把投稿自动发布到远程市场。

Agent 可从项目中选择一张合适的产品截图随投稿提交。截图仅支持 PNG、JPEG 或 WebP，文件不得超过 2 MB。Agent 必须运行相关测试、构建及可用的工作台包校验；无法访问本机投稿接口时，应保留已经验证的投稿数据并如实说明剩余步骤。接收成功后，投稿进入本机待审核队列，状态为 `pending`。“提交成功”只表示已进入待审核队列，不代表审核通过、公开发布或其他用户已经可以安装。
...
工作区侧边栏中的切换和“新建会话”都保留 DSH 原生入口和用户选择的目标。当前有可用的前台工作台时，用户从原生“新建会话”入口创建的会话归属该工作台，并严格创建在用户指定的工作区；创建过程不得恢复该工作台的旧最近会话，也不得改用其他工作区。工作台关闭或不可用时，原生“新建会话”创建普通未绑定会话。切换已有工作区或打开已有普通未绑定会话只更新原生会话区域并保留当前业务面板，不新增或改变归属；只有用户明确点击已绑定工作台的会话时，才唤起对应工作台。异步创建完成前若用户继续导航，宿主可以保存创建时确定的归属，但不得把界面拉回原工作台或覆盖用户后续选择。
...
- 在工作台打开期间通过原生入口新建会话，会话归属创建时的前台工作台、位于用户指定工作区，并且不会被工作台旧最近会话抢回；异步完成不得覆盖用户后续导航。
...
- 市场投稿只接收 GitHub HTTPS 仓库、必填作者与说明以及合规的可选截图；接收后明确显示待审核，不暗示已经公开发布。
- 市场预览和详情展示已有截图、作者及真实可得的安装/点赞数据；数据未知时明确标注未知。

每次提交的新版本都需要验收；个人创建的工作台可以先在本地使用。第二期当前只实现本机投稿接收；GitHub 身份认证、远程投稿传输、官方审核后台及公开市场收录尚未实现，不能将本地可用或本机提交等同于已通过官方收录。
