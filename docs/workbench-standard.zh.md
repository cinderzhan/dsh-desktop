...
工作台内部的原生会话区应支持明确执行“在此工作区新建工作台会话”或“选择资料位置并开始工作台会话”。这类由工作台内部明确发起的动作会创建会话并登记当前工作台归属。需要本地目录时，必须先调用宿主提供的目录选择器，让用户选择已有目录，或由用户在系统选择器中明确新建目录；工作台不得在默认位置、用户主目录、当前仓库或其他推测路径中静默创建文件夹。目录选择完成前不得创建工作区、会话或绑定；用户取消时不得留下目录、空项目或会话。失败可重试，实现不得默默改用无关工作区。当前有前台工作台时，通过 Desktop 原生侧边栏切换工作区也会在目标工作区创建新的当前工作台会话，并在会话成为可见 current 之前登记归属；不得收编目标工作区中已有的普通空白会话、当前工作台旧会话或其他工作台会话。全局“新建会话”和打开普通会话仍属于普通导航，必须保持未绑定并退出工作台。
...
工作区侧边栏中的切换和“新建会话”都保留 DSH 原生入口和用户选择的目标。当前有前台工作台时，原生工作区切换会在用户选择的目标工作区创建一个新的当前工作台会话；宿主必须先持久化会话归属和最近会话，再将它显示为 current，且不得复用或改绑目标中已有的普通会话、当前 owner 会话或其他 owner 会话。全局原生“新建会话”创建的会话始终是新的普通未绑定会话，并退出当前工作台；显式打开已有普通未绑定会话也必须退出。只有用户明确点击已绑定工作台的会话时，才唤起对应工作台。异步创建完成前若用户继续导航、切换工作台或移除 owner，新会话不得绑定到失效 owner，也不得把界面拉回旧工作台或覆盖用户后续选择。

通过全局原生入口新建会话或显式打开普通会话时，宿主必须将 `active` 工作台清空，侧栏工作台入口取消选中，主区域恢复为原生会话。历史 owner 已移除、卸载或暂时不可用的会话也按普通会话处理：不唤起或重装该 owner，并退出当前工作台视图。这个导航只改变前台界面状态，不删除工作台最近会话、业务资料、项目文件、收藏或笔记；再次点击工作台时仍恢复它自己的最近会话。宿主不得给普通会话新增工作台归属，也不得用它更新工作台最近会话。
...
- [ ] 没有会话时能看到业务面板；前台工作台中的原生工作区切换会创建并预先绑定新的 owner 会话；全局新建或打开普通会话会退出工作台且保持未绑定；工作台内创建或恢复的会话归属正确。
