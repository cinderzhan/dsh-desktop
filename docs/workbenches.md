...
Phase 1 adds a local workbench catalog and persistent sidebar entries. Phase 2 begins with local community-submission intake from the market; review, remote catalog publishing, and package upgrades remain later work.

## Phase 2 market-submission MVP

Users start a workbench submission from the market by copying a prepared prompt to the Agent that is developing the workbench. The prompt tells the Agent to finish and validate the package, derive the title, description, author name, and exact HTTPS GitHub repository URL from the project, select an optional safe product screenshot, and then use the existing local submission API. This keeps project inspection and packaging in the Agent's coding context instead of asking the user to re-enter project metadata in a market form.

The Agent must run the repository's relevant tests and build plus `scripts/check-workbench-package.mjs` when available. It should ask the user only for metadata that cannot be verified from the project. When the local endpoint is reachable, it posts `{ title, description, author, repository, screenshot? }` to `/api/desktop-workbenches/submissions` and verifies that the response has an ID and `pending` status. If the endpoint is unavailable, it preserves the validated payload and reports the remaining action without claiming success. The repository URL identifies the proposed source; Desktop does not authenticate with GitHub, upload files to GitHub, modify the repository, or publish the workbench remotely in this MVP.

One product screenshot is optional during submission. It must be a PNG, JPEG, or WebP image no larger than 2 MB. Accepted submissions are stored locally with `pending` review status. “Submitted” therefore means received for review, not published or available to other users.
...
`desktop.workbench.frame` is a single root slot owned by the native conversation panel. Its owner supplies `{ conversation }`. `uiWorkspace.registerSessionOpener` lets the workbench controller route an explicitly selected, bound Session to its installed workbench. `uiWorkspace.registerSessionStarter` applies only to an explicit native New Session action: while an available workbench is active, it creates and binds a new Session for that workbench in the exact workspace selected by the user. It never restores the workbench's recent Session. With no active available workbench, New Session follows the ordinary native unbound flow. Opening an existing workspace or Session does not pass through this starter hook and never acquires a binding merely because a business panel is visible.
...
If native New Session creation finishes after the user navigates elsewhere, Desktop retains the owner captured when creation began but does not reopen that Session, reactivate its workbench, or replace the user's newer navigation. Selecting any existing bound Session still opens its exact available owner.
